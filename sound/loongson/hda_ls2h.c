...
//DEFINE_RAW_SPINLOCK(mailbox_lock);
...
static void __iomem *ls2h_dummy_addr;
/* LS2H writeb */
static void ls2h_writeb(u8 b, volatile void __iomem * addr)
{
	u32 data;
	...
	case 0x143:
	case 0x163:
		data = (b << 24) | (b << 16) | (8 << b) | b;
		writel(data, ls2h_dummy_addr + ((u64) addr & 0xffc));
		writel(data, ls2h_dummy_addr + ((u64) addr & 0xffc));
		break;
	default:
		break;
...
//	u32 check;
	...
	raw_spin_lock_irqsave(&mailbox_lock, flags);
	...
	*(p + 2) = 0;
	...
	raw_spin_unlock_irqrestore(&mailbox_lock, flags);
...
static int azx_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	snd_pcm_group_for_each_entry(s, substream) {
		...
		if (start) {
			...
			/* ls2h: remember LPIB for bogus interrupt fix. */
			azx_dev->fix_prvpos = azx_get_position(chip, azx_dev);
/* ls2h */
#if 0
			if (!rstart)
...
/* ls2h */
	...
	if (pos < (azx_dev->fix_prvpos + runtime->period_size)
	    % runtime->buffer_size) {
		azx_dev->fix_prvpos = pos;
		return -1;
	}
...
static int __devinit azx_probe(struct platform_device *devptr)
{
	static int dev;
	struct snd_card *card;
	struct azx *chip;
...
#ifdef CONFIG_CPU_LOONGSON2H
	/* ls2h */
	ls2h_dummy_addr = ioremap_nocache(0x0EFFF000, 0x1000);
#endif

#ifdef CONFIG_CPU_LOONGSON3
	ls2h_mailbox_addr = ioremap_nocache(0x1bd80120, 0x10);
	/* init mailbox with zero */
#if 1
	p = (volatile u32 __iomem *)ls2h_mailbox_addr;
	p[3] = 0x0; // set stop bit for hda access request to ls2h
#endif
...
static int __devexit azx_remove(struct platform_device *devptr)
{
