...
start:
	...
	if (ls2h_i2c_readb(LS2H_I2C_SR_REG) & SR_NOACK) {
		...
		while (retry--) 
			goto start;
		printk(KERN_CRIT "%s:There is no i2c device ack\n",__func__);
		return 0;
	}
	return 1;
}
...
static void ls2h_i2c_init(struct ls2h_i2c_dev *dev)
{
        ...
        ls2h_i2c_writeb(0x64, LS2H_I2C_PRER_LO_REG);
        ls2h_i2c_writeb(0, LS2H_I2C_PRER_HI_REG);
...
ls2h_i2c_probe(struct platform_device *pdev)
{
	struct ls2h_i2c_dev	*dev;
...
static int
ls2h_i2c_remove(struct platform_device *pdev)
{
