...
struct dm_snapshot {
	...
	struct dm_exception_table pending;	//待处理例外
	struct dm_exception_table complete;//已经完成例外
...
static int snapshot_ctr(struct dm_target *ti, unsigned int argc, char **argv)
{
	...
	//<orign_dev><COM-dev><p/n><chunk_size>
	if (argc != 4) {
	...
	//dm_add_exception函数功能:例外添加到快照私有数据描述符。
...
参数:
返回值:
说明:主要完成两个任务:1.将例外从快照的待处理例外HASH表转移到已经完成例外HASH表中。
				  2.处理延迟的bio请求。
*/
static void pending_complete(struct dm_snap_pending_exception *pe, int success)
{
 ...
 out:
	dm_remove_exception(&pe->e);//从待处理例外HASH表中删除
	snapshot_bios = bio_list_get(&pe->snapshot_bios);
	origin_bios = bio_list_get(&pe->origin_bios);
	free_pending_exception(pe);//销毁待处理例外
	...
	if (error)
		...
		flush_bios(snapshot_bios);//提交快照bio
	//提交快照源的bio
...
static void start_copy(struct dm_snap_pending_exception *pe)
{
	...
	//提交内核拷贝任务，copy_callback为拷贝完成后的回调函数。
	/* Hand over to kcopyd */
 ...
 */
/**ltl
功能:将快照源的数据写到快照链表中的所有快照中。
参数:
返回值:
说明:对于快照源的快照列表中的每一个快照，如果未满，在已经完成例外HASH表中查找对应该chunk项。如果没有找到，查找或插入一个待处理的例外。
如果失败，表示快照已经满。否则已经有一个完整的pe列表，调用start_copy开始复制操作。	
*/
			  ...
			  struct bio *bio)
{
	...
	list_for_each_entry (snap, snapshots, list) {
		 ...
		 */
		//chunk出现在已经完成例外HASH表中，说明源设备中的数据已经被复制过，则直接提交IO
		e = dm_lookup_exception(&snap->complete, chunk);
		if (e)
			goto next_snapshot;
		//查找chunk是否已经出现在待处理例外HASH表中。如果没有，分配一个新的待完成例外对象。
		pe = __lookup_pending_exception(snap, chunk);
		if (!pe) {
			//释放快照锁
			up_write(&snap->lock);
		   /*分配待完成例外对象
			*注:在分配时，要先释放snap->lock锁。因为在分配时，由于已经解锁，快照对象的成员可能已经变化。
			*因此分配完成后，要先获得snap->lock锁，再去检测快照的有效性。
			*/
 ...
 next_snapshot:
		...
		if (pe_to_start_now) {
			start_copy(pe_to_start_now);//开始执行拷贝
			pe_to_start_now = NULL;//为了处理下一个快照做准备。
		}
	}
		      ...
		      union map_info *map_context)
{
	...
	//屏障IO的请求，且数据长度为空，表示此bio是一个刷新请求。
	if (unlikely(bio_empty_barrier(bio)))
		return DM_MAPIO_REMAPPED;
	//要是读操作，直接把bio请求发送给设备成员，如果是写操作，则调用do_origin函数，这个函数为快照源的所有快照构建例外信息
