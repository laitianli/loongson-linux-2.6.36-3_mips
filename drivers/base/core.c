...
参数:
返回值:
说明:在调用此函数前要先关联总线，即要对device.bus赋值，如果有设备类的话，也要对device.class赋值
*/
...
void device_unregister(struct device *dev)
{
	pr_debug("device: '%s': %s\n", dev_name(dev), __func__);
	device_del(dev);
	/*放在最后做是，是防止在del过程还没有完成时，就释放了device*/
	put_device(dev);
}
/*获取下一下设备*/
...
参数:
返回值:
说明:主要是创建/sys的目录:devices、dev和dev目录下的两个子目录:block、char
*/
int __init devices_init(void)
{
	...
	if (!dev_kobj)
		goto dev_kobj_err;
	//创建/sys/devices/block目录
	sysfs_dev_block_kobj = kobject_create_and_add("block", dev_kobj);
	if (!sysfs_dev_block_kobj)
		goto block_kobj_err;
	//创建/sys/device/char目录
