...
static int ls2h_init_regs(struct fb_info *info)
{
	...
	/* reset pstiev */
	ls2h_writew((out | 0xc0000000), LS2H_PIXCLK0_CTRL0_REG);
	ls2h_writew((out | 0xc0000000), LS2H_PIXCLK1_CTRL0_REG);
	/* wait 10000ns */
	for (j = 1; j <= 1000; j++)
		ls2h_readw(LS2H_CHIP_SAMP0_REG);
	/* set pstdiv */
	ls2h_writew((out | 0x80000000), LS2H_PIXCLK0_CTRL0_REG);
	ls2h_writew((out | 0x80000000), LS2H_PIXCLK1_CTRL0_REG);
	/* wait 10000ns */
	for (j = 1; j <= 1000; j++)
		ls2h_readw(LS2H_CHIP_SAMP0_REG);

	/* pll_powerdown */
	ls2h_writew((out | 0x00000080), LS2H_PIXCLK0_CTRL0_REG);
	ls2h_writew((out | 0x00000080), LS2H_PIXCLK1_CTRL0_REG);
	/* wait 10000ns */
	for (j = 1; j <= 1000; j++)
		ls2h_readw(LS2H_CHIP_SAMP0_REG);
	/* pll_powerup set pll*/
...
/* VGA */
	...
	ls2h_writew((hfl << 16) | hr, LS2H_FB_HDISPLAY_VGA_REG);
	ls2h_writew(0x40000000 | (hse << 16) | hss, LS2H_FB_HSYNC_VGA_REG);
	ls2h_writew((vfl << 16) | vr, LS2H_FB_VDISPLAY_VGA_REG);
	ls2h_writew(0x40000000 | (vse << 16) | vss, LS2H_FB_VSYNC_VGA_REG);

	switch (depth) {
	case 32:
	case 24:
		ls2h_writew(0x00100104, LS2H_FB_CFG_VGA_REG);
		ls2h_writew((hr * 4 + 255) & ~255, LS2H_FB_STRI_VGA_REG);
		break;
	case 16:
		ls2h_writew(0x00100103, LS2H_FB_CFG_VGA_REG);
		ls2h_writew((hr * 2 + 255) & ~255, LS2H_FB_STRI_VGA_REG);
		break;
	case 15:
...
/* cursor */
	...
	ls2h_writew(0x00050212, LS2H_FB_CUR_CFG_REG);
...
{
	unsigned int tmp = ls2h_readw(LS2H_FB_CUR_CFG_REG);
	tmp &= ~0xff;
	ls2h_writew(mode ? (tmp | 0x12) : (tmp | 0x10), 
			LS2H_FB_CUR_CFG_REG);
}
...
{
	u32 hot = (x << 16) | (y << 8);	
	u32 con = ls2h_readw(LS2H_FB_CUR_CFG_REG) & 0xff;
	ls2h_writew(hot | con, LS2H_FB_CUR_CFG_REG);
}
...
static int ls2h_fb_cursor(struct fb_info *info, struct fb_cursor *cursor)
{
	...
	if (cursor->set & FB_CUR_SETPOS) {
		...
		ls2h_writew(tmp, LS2H_FB_CUR_LOC_ADDR_REG);
	}
...
static int ls2h_fb_probe(struct platform_device *dev)
{
	...
	par->pdev = dev;
	par->irq = irq;
	par->i2c_adpa_id = (unsigned long)platform_get_drvdata(dev);
