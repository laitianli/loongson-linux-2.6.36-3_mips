...
static int sata_pmp_eh_recover(struct ata_port *ap)
{
 ...
 retry:
	/* PMP attached? */
	if (!sata_pmp_attached(ap)) { /* sata_pmp_attached(ap)=0 */
		/* 错误恢复 */
...
{
	ata_eh_autopsy(ap); /* 错误自分析(对AHCI不采用此流程) */
	ata_eh_report(ap);  /* 串口打印错误日志 */
	sata_pmp_eh_recover(ap); /* AHCI接口驱动分析错误 */
	ata_eh_finish(ap);		/*  */
}
