			      ...
			      unsigned int mask_port_map)
{
	...
	ahci_enable_ahci(mmio); /* 使能AHCI控制器 */
 ...
 * 参数:
 * 返回值:
 * 说明:1.使端口停止工作；2.清除端口的错误寄存器的值；3.清除端口中断；4.开启端口中断
 */
...
void ahci_init_controller(struct ata_host *host)
{
	...
	writel(tmp | HOST_IRQ_EN, mmio + HOST_CTL); /* 开启全局中断 */
	tmp = readl(mmio + HOST_CTL);
	VPRINTK("HOST_CTL 0x%x\n", tmp);
}
