...
static void synopGMAC_linux_cable_unplug_function(synopGMACPciNetworkAdapter *
						  adapter)
{
	u16 data;
...
s32 synopGMAC_check_phy_init(synopGMACPciNetworkAdapter * adapter)
{
	...
	} else
	{
		...
		gmacdev->DuplexMode =
		    (cmd.duplex == DUPLEX_FULL) ? FULLDUPLEX : HALFDUPLEX;
...
static void synopGMAC_linux_powerdown_mac(synopGMACdevice * gmacdev)
{
	...
	synopGMAC_write_wakeup_frame_register(gmacdev,
...
s32 synopGMAC_setup_tx_desc_queue(synopGMACdevice * gmacdev,
				  struct device * dev, u32 no_of_desc,
				  u32 desc_mode)
{
	s32 i;
	...
	if (desc_mode == RINGMODE) {
		...
		first_desc =
		    plat_alloc_consistent_dmaable_memory(dev,
							 sizeof(DmaDesc) *
							 no_of_desc, &dma_addr);
		if (first_desc == NULL) {
		...
		first_desc =
		    plat_alloc_consistent_dmaable_memory(dev, sizeof(DmaDesc),
							 &dma_addr);
		if (first_desc == NULL) {
...
void synopGMAC_giveup_rx_desc_queue(synopGMACdevice * gmacdev,
				    struct device *dev, u32 desc_mode)
{
	s32 i;
	...
	if (desc_mode == RINGMODE) {
		for (i = 0; i < gmacdev->RxDescCount; i++) {
			synopGMAC_get_desc_data(gmacdev->RxDesc + i, &status,
						&dma_addr1, &length1, &data1,
						&dma_addr2, &length2, &data2);
			if ((length1 != 0) && (data1 != 0)) {
				//dma_unmap_single(dev, dma_addr1, ETHER_MAX_LEN,
				dma_unmap_single(dev, dma_addr1, ((struct sk_buff *)data1)->len,
						 PCI_DMA_FROMDEVICE);
			...
			if ((length2 != 0) && (data2 != 0)) {
				//dma_unmap_single(dev, dma_addr2, ETHER_MAX_LEN,
				dma_unmap_single(dev, dma_addr2, ((struct sk_buff *)data2)->len,
						 PCI_DMA_FROMDEVICE);
				dev_kfree_skb((struct sk_buff *)data2);
		...
		for (i = 0; i < gmacdev->RxDescCount; i++) {
			synopGMAC_get_desc_data(first_desc, &status, &dma_addr1,
						&length1, &data1, &dma_addr2,
						&length2, &data2);
			TR("%02d %p %08x %08x %08x %08x %lx %lx\n", i,
			   first_desc, first_desc->status, first_desc->length,
			...
			plat_free_consistent_dmaable_memory(dev, (sizeof(DmaDesc)), first_desc, first_desc_dma_addr);
				    ...
				    struct device *dev, u32 desc_mode)
{
	...
	if (desc_mode == RINGMODE) {
		for (i = 0; i < gmacdev->TxDescCount; i++) {
			synopGMAC_get_desc_data(gmacdev->TxDesc + i, &status,
						&dma_addr1, &length1, &data1,
						&dma_addr2, &length2, &data2);
			if ((length1 != 0) && (data1 != 0)) {
				//dma_unmap_single(dev, dma_addr1, ETHER_MAX_LEN,
...
#ifdef ENH_DESC_8W
		desc_index =
		    synopGMAC_get_tx_qptr(gmacdev, &status, &dma_addr1,
					  &length1, &data1, &dma_addr2,
...
//                      dma_unmap_single(dev,dma_addr1,length1,PCI_DMA_TODEVICE);
			dma_unmap_single(dev,dma_addr1,BUS_SIZE_ALIGN(netdev->mtu + 
							  ETHERNET_PACKET_EXTRA),PCI_DMA_TODEVICE);
			...
			if (synopGMAC_is_desc_valid(status)) {
				...
				adapter->synopGMACNetStats.tx_aborted_errors +=
				    synopGMAC_is_tx_aborted(status);
				adapter->synopGMACNetStats.tx_carrier_errors +=
	...
	do {
#ifdef ENH_DESC_8W
		desc_index =
		    synopGMAC_get_rx_qptr(gmacdev, &status, &dma_addr1, NULL,
		...
		if (desc_index >= 0 && data1 != 0) {
			...
			if (synopGMAC_is_rx_desc_valid(status)) {
				...
				dma_unmap_single(dev,dma_addr1,BUS_SIZE_ALIGN(netdev->mtu
								  + ETHERNET_PACKET_EXTRA) ,PCI_DMA_FROMDEVICE);
				...
				if (synopGMAC_is_rx_checksum_error
				    (gmacdev, status) == RxIpHdrChkError) {
					//Linux Kernel doesnot care for ipv4 header checksum. So we will simply proceed by printing a warning ....
					TR(" Error in 16bit IPV4 Header Checksum <Chk Status = 6>  \n");
					skb->ip_summed = CHECKSUM_UNNECESSARY;	//Let Kernel bypass the TCP/UDP checksum computation
				}
				...
				if (synopGMAC_is_rx_checksum_error
				    (gmacdev,
				     status) == RxIpHdrPayLoadChkBypass) {
					TR("Ip header and TCP/UDP payload checksum Bypassed <Chk Status = 1>\n");
					skb->ip_summed = CHECKSUM_NONE;	//Let Kernel compute the Checksum
				}
				if (synopGMAC_is_rx_checksum_error
				...
				if (synopGMAC_is_rx_checksum_error
				    (gmacdev, status) == RxPayLoadChkError) {
					TR(" TCP/UDP payload checksum Error <Chk Status = 5>  \n");
					skb->ip_summed = CHECKSUM_NONE;	//Let Kernel compute the Checksum
				}
				if (synopGMAC_is_rx_checksum_error
			...
			skb =
			    alloc_skb(BUS_SIZE_ALIGN
				      (netdev->mtu + ETHERNET_PACKET_EXTRA) + 2,
			...
			dma_addr1 =
			    dma_map_single(dev, skb->data,
					   BUS_SIZE_ALIGN(netdev->mtu +
							  ETHERNET_PACKET_EXTRA),
					   PCI_DMA_FROMDEVICE);
			desc_index =
			    synopGMAC_set_rx_qptr(gmacdev, dma_addr1,
						  BUS_SIZE_ALIGN(netdev->mtu +
								 ETHERNET_PACKET_EXTRA),
						  (u64) skb, 0, 0, 0);
...
irqreturn_t synopGMAC_intr_handler(s32 intr_num, void *dev_id)
{
	...
	if (interrupt & synopGMACDmaError) {
		...
		synopGMAC_set_mac_addr(gmacdev, GmacAddr0High, GmacAddr0Low,
				       mac_addr0);
			...
			do {
				...
				dma_addr =
				    dma_map_single(dev, skb->data,
						   ...
						   PCI_DMA_FROMDEVICE);
				status =
				    synopGMAC_set_rx_qptr(gmacdev, dma_addr,
							  BUS_SIZE_ALIGN
							  (netdev->mtu +
							   ETHERNET_PACKET_EXTRA),
							  (u64) skb, 0, 0, 0);
				...
				if (status < 0)
					...
					dma_unmap_single(dev,dma_addr,BUS_SIZE_ALIGN(netdev->mtu
								  + ETHERNET_PACKET_EXTRA) ,PCI_DMA_FROMDEVICE);
...
static int mac_read(int bus_id, int addr, unsigned char *buf)
{
...
static int mac_write(int bus_id, int addr, unsigned char *buf)
{
...
s32 synopGMAC_linux_open(struct net_device * netdev)
{
	...
	synopGMAC_attach(adapter->synopGMACdev,
			 adapter->synopGMACMappedAddr + MACBASE,
			 adapter->synopGMACMappedAddr + DMABASE,
			 DEFAULT_PHY_BASE, netdev->dev_addr);

	/*Lets read the version of ip in to device structure */
	synopGMAC_read_version(gmacdev);

	synopGMAC_get_mac_addr(adapter->synopGMACdev, GmacAddr0High,
			       GmacAddr0Low, netdev->dev_addr);
	...
	{
		unsigned short data;
		synopGMAC_read_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 2,
				       &data);
	...
	if (request_irq
	    (adapter->irq, synopGMAC_intr_handler, IRQF_SHARED, netdev->name,
	     netdev)) {
	...
	synopGMAC_setup_tx_desc_queue(gmacdev, dev, TRANSMIT_DESC_SIZE,
				      RINGMODE);
#else
	synopGMAC_setup_tx_desc_queue(gmacdev, dev, TRANSMIT_DESC_SIZE,
				      CHAINMODE);
...
#ifndef USE_CHAIN_MODE
	synopGMAC_setup_rx_desc_queue(gmacdev, dev, RECEIVE_DESC_SIZE,
				      RINGMODE);
#else
	synopGMAC_setup_rx_desc_queue(gmacdev, dev, RECEIVE_DESC_SIZE,
				      CHAINMODE);
 ...
 */
s32 synopGMAC_linux_xmit_frames(struct sk_buff * skb,
				struct net_device * netdev)
{
	s32 status = 0;
	...
	wmb();
	status =
	    synopGMAC_set_tx_qptr(gmacdev, dma_addr, skb->len, (u64) skb, 0, 0,
				  0, offload_needed);
	if (status < 0) {
		TR0("%s No More Free Tx Descriptors\n", __FUNCTION__);
		dma_unmap_single(dev,dma_addr,skb->len,PCI_DMA_TODEVICE);
		//dev_kfree_skb_irq(skb);
		dev_kfree_skb(skb);
		spin_unlock_irqrestore(&adapter->lock, flags);
		return -EBUSY;
...
struct net_device_stats *synopGMAC_linux_get_stats(struct net_device *netdev)
{
	TR("%s called \n", __FUNCTION__);
	return (&
		(((synopGMACPciNetworkAdapter *) (netdev_priv(netdev)))->
		 synopGMACNetStats));
}
...
s32 synopGMAC_linux_set_mac_address(struct net_device * netdev, void *macaddr)
{
	...
	gmac_id = adapter->pdev->id;
	i2c_bus_id = adapter->i2c_bus_id;
	...
	if (!mac_write(i2c_bus_id, 6 * gmac_id, addr->sa_data))
		TR0("%s:%d write mac address to eeprom error!\n", __func__,
		       __LINE__);

	synopGMAC_set_mac_addr(gmacdev, GmacAddr0High, GmacAddr0Low,
			       addr->sa_data);
	synopGMAC_get_mac_addr(gmacdev, GmacAddr0High, GmacAddr0Low,
			       netdev->dev_addr);
			     ...
			     s32 cmd)
{
	...
	case IOCTL_READ_REGISTER:	//IOCTL for reading IP registers : Read Registers
		if (req->unit == 0)	// Read Mac Register
			req->data =
			    synopGMACReadReg((u32 *) gmacdev->MacBase,
					     req->addr);
		else if (req->unit == 1)	// Read DMA Register
			req->data =
			    synopGMACReadReg((u32 *) gmacdev->DmaBase,
					     req->addr);
	...
	case IOCTL_WRITE_REGISTER:	//IOCTL for reading IP registers : Read Registers
		if (req->unit == 0)	// Write Mac Register
			synopGMACWriteReg((u32 *) gmacdev->MacBase, req->addr,
					  req->data);
		else if (req->unit == 1)	// Write DMA Register
			synopGMACWriteReg((u32 *) gmacdev->DmaBase, req->addr,
					  req->data);
	...
	case IOCTL_READ_RXDESC:	//IOCTL for Reading Rx DMA DESCRIPTOR
		memcpy(ifr->ifr_data,
		       gmacdev->RxDesc + ((DmaDesc *) (ifr->ifr_data))->data1,
		       sizeof(DmaDesc));
		break;

	case IOCTL_READ_TXDESC:	//IOCTL for Reading Tx DMA DESCRIPTOR
		memcpy(ifr->ifr_data,
		       gmacdev->TxDesc + ((DmaDesc *) (ifr->ifr_data))->data1,
		       sizeof(DmaDesc));
		break;
...
static void gmac_get_regs(struct net_device *dev, struct ethtool_regs *regs,
			  void *regbuf)
{
	synopGMACPciNetworkAdapter *adapter = netdev_priv(dev);
	synopGMACdevice *gmacdev = adapter->synopGMACdev;
...
static void gmac_get_strings(struct net_device *netdev, u32 stringset,
			     u8 * data)
{
	switch (stringset) {
	case ETH_SS_STATS:
...
/* Maybe the ethaddr will be passed from pmon */
#if 0
 ...
 * \return Returns 0 on success and Error code on failure.
 */
void *synopGMAC_init_network_interface(struct platform_device *pdev,
				       u8 * synopGMACMappedAddr,
				       u32 synopGMACMappedAddrSize, int irq)
{
	s32 err;
	int gmac_id;
	int i2c_bus_id;
	...
	synopGMACadapter->synopGMACdev =
	    (synopGMACdevice *) plat_alloc_memory(sizeof(synopGMACdevice)); // not sure whether this alloc should be used?
	if (!synopGMACadapter->synopGMACdev) {
		TR0("Error in Memory Allocataion \n");
	}

	gmac_id = pdev->id;
	i2c_bus_id = (long)platform_get_drvdata(pdev);
	synopGMACadapter->i2c_bus_id = i2c_bus_id;
	if (!mac_read(i2c_bus_id, 6 * gmac_id, hwaddr)) {
		TR0("Get random Mac Address!\n");
		random_ether_addr(hwaddr);
	}
	memcpy(netdev->dev_addr, hwaddr, 6);

/* Maybe the ethaddr will be passed from pmon in the further */
#if 0
	if (1) {
		static int index = 0;
		if (!ether_set) {
	...
	synopGMAC_attach(synopGMACadapter->synopGMACdev,
			 synopGMACMappedAddr + MACBASE,
			 synopGMACMappedAddr + DMABASE, DEFAULT_PHY_BASE,
			 netdev->dev_addr);
	...
	synopGMACadapter->mii.supports_gmii =
	    mii_check_gmii_support(&synopGMACadapter->mii);
