...
/*-------------------------------------------------------------------
 *              tl8111dl EEPROM read/write function
 *-------------------------------------------------------------------*/

u16 rtl_eeprom_read_sc(struct rtl8169_private *tp, u16 reg);

void rtl_eeprom_type(struct rtl8169_private *tp)
{
    void __iomem *ioaddr=tp->mmio_addr;
    u16 magic;
...
#if 1
	if (tp->mac_version == 25) { /* Fixed the RTL8111DL MAC address error */
