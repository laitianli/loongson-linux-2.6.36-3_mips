...
static int ls2h_platform_init(void)
{
	...
	ls2h_writew(tmp | (1 << 26) | (1 << 14) | (1 << 4), LS2H_CHIP_CFG0_REG);

	platform_set_drvdata(&ls2h_gmac0_device, (void *)0);
	platform_set_drvdata(&ls2h_gmac1_device, (void *)0);
	platform_set_drvdata(&ls2h_dc_device, (void *)1);

	tmp = ls2h_readw(LS2H_GPIO_IN_REG);
	tmp = (tmp >> 8) & 0xf;
	if (tmp == LS2H_BOARD_VER_OLD) {
		platform_set_drvdata(&ls2h_gmac0_device, (void *)1);
		platform_set_drvdata(&ls2h_gmac1_device, (void *)1);
	}
	...
	ls2h_writew((1 << 0x1) | (1 << 12), LS2H_LPC_CFG1_REG);
...
device_initcall(ls2h_platform_init);
