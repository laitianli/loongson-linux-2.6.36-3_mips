...
static int show_cpuinfo(struct seq_file *m, void *v)
{
...
#endif
#if defined(CONFIG_CPU_LOONGSON3) || defined(CONFIG_CPU_LOONGSON2H)
	cpu_mhz = (cpu_data[n].udelay_val / (500000/HZ))*100 + (cpu_data[n].udelay_val / (5000/HZ)) % 100;
#endif
	/*
	...
	sprintf(fmt, "model name\t\t: %%s V%%d.%%d%s\n",
...
#if defined(CONFIG_CPU_LOONGSON3) || defined(CONFIG_CPU_LOONGSON2H)
	seq_printf(m, "cpu MHz\t\t\t: %u.%02u\n", (cpu_mhz + (cpu_mhz / 4)) / 100, (cpu_mhz + (cpu_mhz / 4)) % 100);
#endif
