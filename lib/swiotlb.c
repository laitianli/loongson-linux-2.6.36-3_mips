...
/* make io_tlb_overflow tunable too? */
  ...
  /*swiotlb_tbl is just used for driver/gpu modlue.
...
#if defined (CONFIG_CPU_LOONGSON3) || defined (CONFIG_CPU_LOONGSON2H)
	if (dma64_supported && hwdev && hwdev->coherent_dma_mask)
#else
			    ...
			    struct dma_attrs *attrs)
{
	...
	if (!dma_capable(dev, dev_addr, size))
		panic("map_single: bounce buffer is not DMA'ble dev_addr:0x%llx", dev_addr);
